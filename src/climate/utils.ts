...
export const MODE_COLORS_LIGHT: Record<string, { color: string; colorTo: string }> = {
  heat: {
    ...
    colorTo:
      "oklch(0.62 0.20 50)" /* warmer second stop keeps the gradient reading as heat, not just red */,
  },
  cool: {
    color:
      "oklch(0.70 0.16 230)" /* deep enough L so blue at 22% alpha does not wash into the light surface */,
    colorTo:
      "oklch(0.74 0.14 200)" /* cyan-ish second stop adds depth without breaking the cool identity */,
  },
  heat_cool: {
    color:
      "oklch(0.70 0.16 150)" /* balanced green-cyan reads as auto/dual-mode at low alpha on light */,
    colorTo:
      "oklch(0.68 0.16 165)" /* slight hue shift keeps the gradient interesting without changing identity */,
  },
  auto: {
    color:
      "oklch(0.70 0.16 150)" /* alias of heat_cool — HVAC_MODES exposes both keys for HA compat */,
    colorTo: "oklch(0.68 0.16 165)" /* alias of heat_cool — see above */,
  },
  dry: {
    color:
      "oklch(0.78 0.15 85)" /* warm yellow stays readable at 22% alpha; deeper L would muddy at low alpha */,
    colorTo: "oklch(0.76 0.16 65)" /* amber second stop reinforces the moisture-removal identity */,
  },
  fan_only: {
    color:
      "oklch(0.76 0.13 195)" /* desaturated cyan reads as airflow without competing with cool mode */,
...
export const MODE_COLORS_DARK: Record<string, { color: string; colorTo: string }> = {
  ...
  cool: {
    color:
      "oklch(0.74 0.16 230)" /* +0.04 L lifts blue against dark bg so 22% alpha does not go muddy */,
    colorTo: "oklch(0.78 0.14 200)" /* +0.04 L parity keeps the gradient legible on dark */,
  },
  heat_cool: {
    color:
      "oklch(0.74 0.16 150)" /* +0.04 L lift keeps the balanced green-cyan reading dual-mode on dark */,
    colorTo: "oklch(0.72 0.16 165)" /* +0.04 L parity with light variant */,
  },
  auto: {
    color:
      "oklch(0.74 0.16 150)" /* alias of heat_cool — HVAC_MODES exposes both keys for HA compat */,
    colorTo: "oklch(0.72 0.16 165)" /* alias of heat_cool — see above */,
  },
  dry: {
    color:
      "oklch(0.82 0.15 85)" /* +0.04 L: amber needs the lift so it reads warm-yellow, not olive, on dark */,
    colorTo:
      "oklch(0.80 0.16 65)" /* +0.04 L parity, slight chroma bump for the amber second stop */,
  },
  fan_only: {
    color:
      "oklch(0.80 0.13 195)" /* +0.04 L makes the desaturated cyan still register as airflow on dark */,
